...
const AiContentResponse = ({content}) => {
    if (!content) return null;
    
  return (
    <div className='max-w-4xl mx-auto'>
        <div className='text-[14px] prose prose-stale dark:prose-invert max-w-none'>
            <ReactMarkdown
                ...
                components={{
                    ...
                    blockquote({children}){
                        return <blockquote className='border dark:border-gray-700 border-gray-200 pl-4 italic my-4'>{children}</blockquote>
                    },
                    h1({children}){
                    ...
                    a({children}){
                        return <a className='text-blue-600 dark:bg-blue-200 hover:underline'>{children}</a>
                    },
                    table({children}){
                        return(
                            <div className='overflow-x-auto my-4'>
                                 <table className='min-w-full divide-y divide-grey-300 border border-grey-200'>{children}</table> 
                            </div>
...
function CodeBlock({code,language}){
    ...
    return <div className='relative my-6 rounded-lg overflow-hidden dark:bg-gray-900 bg-gray-50 border dark:border-gray-700 border-gray-200'>
        <div className='flex items-center justify-between px-4 py-2 dark:bg-gray-700 bg-gray-100 border-b border-gray-200'>
            <div className='flex items-center space-x-2'>
                <LuCode size={16} className='dark:text-gray-100 text-gray-600'/>
                <span className='text-xs font-semibold dark:text-gray-100 text-gray-600 uppercase tracking-wide'>
                    {language || 'Code'}
                </span>
            </div>
            <button 
                onClick={copyCode}
                className='text-gray-500 cursor-pointer dark:text-gray-100 dark:hover:text-gray-300 hover:text-gray-700 focus:outline-none relative group'
...
export default AiContentResponse
